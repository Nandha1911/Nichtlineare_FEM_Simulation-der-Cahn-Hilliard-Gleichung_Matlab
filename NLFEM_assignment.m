...
cu=0.75;
Mo=0.3;                                          
l=10;  %Make them as input function length
h=input("enter the size of the element :\n");           %Assigned as h=0.1 as per given constants
elements=0:h:l;
...
lambda=0.01375;
x=[0,h];
z=1;  %columns in assembly
zz=4;  %columns in assembly

xi=[-0.861136311594053,-0.339981043584856,0.339981043584856,0.861136311594053]; %Gauss quadrature values
w=[0.347854845137454,0.652145154862546,0.652145154862546,0.347854845137454];
%%Hermite Shape function (anonymous)
Nherm= @ (xi) [1/4*((2+xi)*((xi-1)^2)),h/8*((xi+1)*((xi-1)^2)),1/4*((2-xi)*((xi+1)^2))...
    ,h/8*((xi-1)*((xi+1)^2))];

%%Lagrangian shape function
Nlagr= [1/2*(1-xi),1/2*(1+xi)];         %Shape function of Lagrangian
DNlagr = [-1/2,1/2];                    %first derivative of shape function of lagrangian

Jac=DNlagr*transpose(x);                %Jacobi value to substitute in B and G matrix

%%Newton-Raphson counter
...
for m=1:length(t)
       ...
       Rglobnew=zeros(nodes,1);
            for n=1:(length(elements)-1)            
                ...
                Rcapele=transpose(A)*Res(Nherm,ccapnew,ccapold,xi,Jac,h,Psinot,chi,lambda,Mo,w,dt); %to assemble in global Rcap
                temp2K=K;
                Kglob=Kglob+temp2K;
                temp2R=Rcapele;
                Rglobnew=Rglobnew+temp2R;
                z=z+2;
                zz=zz+2;
             end
         dccap=Kglob\(-Rglobnew);
         cglobnew=cglobnew+dccap;
         ii=ii+1;               %initialised for newton-raphson counter
         disp(norm(dccap,inf));
         disp(1e-5*norm(cglobnew,inf));
         disp(norm(Rglobnew,inf));
         disp(0.005*max(norm(Rglobold,inf),1e-8));
        if (norm(dccap,inf) < 1e-5*(norm(cglobnew,inf) && norm(Rglobnew,inf) < 0.005*max(norm(Rglobold,inf),1e-8))) 
            break;
        end
       Rglobold=Rglobnew;
       end
...
ylim([0 1]);
xlabel("Length (in mm) ");
ylabel("concenteration");
hold on;
plot((0:h:l),cinitial(1:2:end));
ylim([0 1]);
legend("Initial concentration val","Attained concenteration val")
figure (2);
plot(energy);
...
function R =Res(Nherm,ccapnew,ccapold,xi,Jac,h,Psinot,chi,lambda,Mo,w,dt)  %Re to calculate per element
 R=zeros(4,1);
  for i=1:4
   Nherm1=Nherm(xi(i));
   c=Nherm1*ccapnew;
   ...
   Q1=transpose(Nherm(xi(i)))*Nherm(xi(i))*((ccapnew-ccapold)/dt);
...
Kt=zeros(4,4);
  for i=1:4
   ...
   k2=(mobility1(c,Mo)*freeenergy2(c,Psinot,chi)*cdash*(transpose(B(xi,i,Jac,h))*Nherm1))+(mobility(c,Mo)...
       *freeenergy3(c,Psinot)*cdash*(transpose(B(xi,i,Jac,h))*Nherm1))+(mobility(c,Mo)*freeenergy2(c,Psinot,chi)*(transpose(B(xi,i,Jac,h))*B(xi,i,Jac,h)));
   k3=(lambda*mobility2(c,Mo)*cdodash*cdash*(transpose(B(xi,i,Jac,h))*Nherm1))+(lambda*mobility1(c,Mo)*cdash*...
       (transpose(B(xi,i,Jac,h))*G(xi,i,Jac,h)))+...
       (lambda*mobility1(c,Mo)*cdodash*(transpose(B(xi,i,Jac,h))*B(xi,i,Jac,h)));
   k4=(lambda*mobility1(c,Mo)*cdodash*(transpose(G(xi,i,Jac,h))*Nherm1))+(lambda*mobility(c,Mo)*(transpose(G(xi,i,Jac,h))*G(xi,i,Jac,h)));
...
function B = B(xi,i,Jac,h)
     DNherm = [(3/4)*( (xi(i)^2) - 1),(h/8)*( (3*(xi(i)^2))-(2*xi(i))+1),(-3/4)*((xi(i)^2)-1),...
         (h/8)*( (3*(xi(i)^2))+(2*xi(i))-1 )];  %first derivative of shape function of Hermite
     B=DNherm*inv(Jac); 
end

function G = G(xi,i,Jac,h)
     DNhermsqr=[3/2*(xi(i)),(h/4)*((3*xi(i))-1),(-3/2)*xi(i),(h/4)*((3*xi(i))+1)]; %Second derivative of...
     % shape function of Hermite
